...
router.post("/", requireRole("admin"), async (req, res) => {
  if (!pool) return res.status(503).json({ error: "Banco de dados não configurado" });
  const { name, categoryId, description, price, imageUrl, promoPrice, promoActive, promoDays, manualCost } = req.body || {};
  if (!name || !categoryId || !price) return res.status(400).json({ error: "Nome, categoria e preço são obrigatórios" });
  const { rows } = await pool.query(
    `insert into products(name,category_id,description,price,image_url,promo_price,promo_active,promo_days,manual_cost)
     values($1,$2,$3,$4,$5,$6,$7,$8,$9) returning *`,
    [name, categoryId, description || null, price, imageUrl || null, promoPrice || null, !!promoActive, promoDays || null, manualCost || null]
  );
  res.status(201).json(rows[0]);
});
...
router.patch("/:id", requireRole("admin"), async (req, res) => {
  if (!pool) return res.status(503).json({ error: "Banco de dados não configurado" });
  const { name, categoryId, description, price, imageUrl, active, promoPrice, promoActive, promoDays, manualCost } = req.body || {};
  const { rows } = await pool.query(
    `update products set
       ...
       promo_days=coalesce($9,promo_days), manual_cost=coalesce($10,manual_cost)
     where id=$11 returning *`,
    [name, categoryId, description, price, imageUrl, active, promoPrice, promoActive, promoDays, manualCost, req.params.id]
  );
  res.json(rows[0] || {});
});
